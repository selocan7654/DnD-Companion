...
**UAT:** `docs/11_UAT.md` **MVP'de yok** — audit boyutu D12 opsiyonel; kullanıcı açıkça UAT istemedikçe atla.

**Faz eşleme:** `53-phase-04-dm-screen-admin.mdc` → fix `53-phase-04-dm-screen-admin-fix.mdc` (aynı `NN`).
...
```markdown
**Anladıklarım:** Faz 4 DM Screen branch'inde audit isteniyor; kaynak `@53-phase-04-dm-screen-admin`.
**Soru:** Audit kapsamı tam faz mı, yoksa belirli iterasyon(lar) mı (örn. yalnızca İterasyon 5–7)?
```
...
Her bulguya benzersiz ID: `F-<XX>-<NNN>` (ör. `F-04-001`).

| Severity | Ne zaman |
| -------- | -------- |
| **BLOCKER** | Güvenlik baseline ihlali; kritik API/ekran yok; Explicit Don'ts ihlali; CI kırmızı (lint/typecheck/test) |
| **HIGH** | Done Definition maddesi eksik/hatalı; yetki deny integration test yok; guard'sız yazma endpoint'i |
| **MEDIUM** | Pattern sapması (policy eksik tek endpoint); kısmi implementasyon |
...
| UAT (`docs/11_UAT.md`) | **MVP'de yok** — opsiyonel; kullanıcı istemedikçe audit boyutu D12 atlanır |
| `48-git-phase-branch` | Fix oturumları aynı faz branch veya `fix/F<N>-*` branch — fix `.mdc`'de belirtilir |
| Human Gate (`docs/10` §1) | Controller Human Gate'i **tamamlamaz**; girdi sağlar |
