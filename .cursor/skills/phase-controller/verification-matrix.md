...
| D3 | API contract | `docs/03` | Controller, DTO, `/api/v1` prefix |
| D4 | Ekran kataloğu | `docs/06` S-* | routes, pages, AuthGuard (UX) |
| D5 | DB şema | `docs/02` | `schema.prisma`, migrations |
| D6 | Güvenlik baseline | `03`, `docs/07` | JwtAuthGuard, policy, EmailVerifiedGuard |
| D7 | Explicit Don'ts | phase `.mdc` | Anti-pattern taraması |
| D8 | Scope creep | phase `.mdc` §Files NOT touched | Yeni dosya vs izinli scope |
| D9 | Test & coverage | `docs/08`, Done Definition | `*.spec.ts`, CI script |
| D10 | Pattern tutarlılığı | `04`, glob 10–17 | Envelope, class-validator, Prisma |
| D11 | Roadmap deliverable | `docs/10` §Faz N | Deliverable listesi |
| D12 | UAT hazırlık | — | **MVP'de yok** — opsiyonel; `docs/11_UAT.md` yoksa ➖ N/A |
...
**Kritik modül eşiği** (`04-quality-gates`): `policies/`, `guards/`, `auth/` → eksik test/coverage = min HIGH.
...
2. `@Controller`, `@Get`, `@Post`, route path grep — prefix `/api/v1`
3. Yazma endpoint'leri: `JwtAuthGuard` + policy (`canUpdate` vb.)
4. `EmailVerifiedGuard` — email doğrulama gerektiren eylemler
5. DTO — `class-validator`; homebrew JSONB — shared Zod `safeParse`
6. Error code — SCREAMING_SNAKE envelope (`docs/03` Bölüm 2)

| Sapma | Tip | Severity |
| ----- | --- | -------- |
| Endpoint yok | MISSING | BLOCKER (core) / HIGH |
| Guard/policy eksik (yazma) | SECURITY | BLOCKER |
| Yetkisiz kaynak 403 yerine 200 | SECURITY | HIGH |
...
2. `apps/web/src/` routes + pages
3. AuthGuard / koşullu render — UX only notu; backend deny ayrı D6
4. UI metinleri İngilizce spot-check (opsiyonel LOW)
...
Eksik tablo/kolon → BLOCKER (data model). Index eksik → MEDIUM (defer notu ile).
...
| # | Kontrol | Kanıt |
| --- | ------- | ----- |
| 1 | Authorization | Guard + policy yazma endpoint'lerinde |
| 2 | Data access | Yalnızca Prisma Client; raw SQL yok |
| 3 | Token storage | Access memory; refresh httpOnly cookie; localStorage yok |
| 4 | PII leakage | `password_hash` response yok; email sahip/admin dışı yok |
| 5 | Input boundary | DTO validation; upload png/jpeg/webp, SVG yok |
| 6 | Auth tests | Yetki değişikliği → integration test aynı PR'da |

İhlal → `SECURITY` BLOCKER veya HIGH.

**Admin faz:** `LAST_ADMIN` koruması backend test zorunlu HIGH.
...
2. `08_TESTING_STRATEGY` — yetki matrisi deny senaryosu (integration)
3. CI komut çıktısı (read-only)
4. Coverage eşik: `policies/`, `guards/` %100; `auth/` ≥%95

| Durum | Tip | Severity |
| ----- | --- | -------- |
| Integration test yok (yetki) | TEST_GAP | HIGH |
| Deny senaryosu yok | TEST_GAP | HIGH |
| CI fail | TEST_GAP | BLOCKER |
| Coverage <% eşik (kritik path) | TEST_GAP | HIGH |
...
- `HttpException` envelope + SCREAMING_SNAKE `error`
- Prisma service katmanında; controller ince
- FE: `localStorage`/`sessionStorage` access token yok
- WebSocket: campaign membership join kontrolü (faz WS kapsıyorsa)
...
**MVP'de yok.** `docs/11_UAT.md` projede tanımlı değilse boyut **➖ N/A** — raporda atla.

Kullanıcı açıkça UAT audit isterse ve doc mevcutsa: maddeleri bulgu ile cross-ref; controller UAT dosyasını **güncellemez**.
...
| Full-stack | D1–D11 (D12 opsiyonel — MVP'de yok) |
| Admin | D1–D11 + LAST_ADMIN D6/D7 |
| Security hardening | D6, D7, D9, D11 |
| WebSocket / DM Screen | D3, D6, D10, D11 |
