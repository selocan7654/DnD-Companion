...
**Referans implementasyon:** DnD Companion (`.cursor/rules/` — 4 katman, `50`–`54` faz kuralları). Detay: [layer-taxonomy.md](layer-taxonomy.md), [reference.md](reference.md).
...
| `docs/mimari-kararlar.md` | Sabit kararlar, stack pin (`[KATEGORI-NNN]` ID'ler) |
...
| -- | ------------ | ------------ |
| 00 | Kimlik, stack pin, monorepo, domain terimleri, MVP dışı | `00`, `01` özet, `mimari-kararlar` stack |
| 01 | Vibe coding, MVP scope, test-first, self-review | `10` Bölüm 1, `09` |
| 02 | EN UI / EN code naming, commit, error code | `09` Bölüm 5 |
...
Tipik set: endpoint, ekran, migration, policy, refactor, mimari karar (`45-write-architectural-decision`), failing test, websocket (+ projeye özel 47.x).
...
**Implementasyon sonrası:** `@phase-controller` ile gap audit → `-fix.mdc` (varsayılan çıktı; ayrı fix-report dosyası yok).
```
