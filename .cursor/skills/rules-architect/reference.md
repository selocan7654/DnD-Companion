...
| `docs/mimari-kararlar.md` | 45 how-to; 00 pin kararlar | Karar ID (`[AUTH-004]`) + 1 cümle karar | Tam karar metni kopyası |
...
Yeni framework/library → `docs/mimari-kararlar.md` kararı gerekir (`45-write-architectural-decision.mdc`).
...
## DnD Companion referans indeks

Tam liste: `.cursor/rules/`. Özet:

| Katman | Dosyalar |
| ------ | -------- |
| 00–04 | project-identity, coding-philosophy, language-naming, security-baseline, quality-gates |
| 10–17 | backend-general, auth, authorization, domain-modules, controllers, database-prisma, websocket, admin-audit |
| 20–25 | frontend-general, routes, forms, rtk-query, components, a11y |
| 30, 35 | infrastructure, testing |
| 40–48 | endpoint, screen, migration, authorization-policy, refactor, architectural-decision, fix-test, websocket-live-fields, git-phase-branch |
| 50–54 | phase 01–05 (auth → upload/production) |

Yeni kural yazarken **en yakın komşu** dosyayı `Read` ile aç; ton ve detay seviyesini kopyala — içeriği `docs/`'tan türet.
